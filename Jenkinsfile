pipeline {
    ...
    environment {
        FRONT_SERVER_1 = 'nova-dev@125.6.36.57'
        DEPLOY_PATH_1 = '/home/nova-dev'
        REPO_URL = 'https://github.com/nhnacademy-be6-supernova/novabook-auth.git'
        ARTIFACT_NAME = 'auth-0.0.1-SNAPSHOT.jar'
    ...
    stages {
        ...
        stage('Deploy to Front Server 1') {
            steps {
                deployToServer(FRONT_SERVER_1, DEPLOY_PATH_1, 8778)
        ...
        stage('Verification') {
            steps {
                verifyDeployment(FRONT_SERVER_1, 8778)
            }
        }
    }
    post {
...
def deployToServer(server, deployPath, port) {
    withCredentials([sshUserPrivateKey(credentialsId: 'nova-dev', keyFileVariable: 'PEM_FILE')]) {
        ...
        ssh -o StrictHostKeyChecking=no -i \$PEM_FILE ${server} 'nohup /home/jdk-21.0.3+9/bin/java -jar ${deployPath}/${ARTIFACT_NAME} --server.port=${port} ${env.JAVA_OPTS} > ${deployPath}/auth_app.log 2>&1 &'
        """
    }
}
...
def showLogs(server, deployPath) {
    withCredentials([sshUserPrivateKey(credentialsId: 'nova-dev', keyFileVariable: 'PEM_FILE')]) {
        sh """
        ssh -o StrictHostKeyChecking=no -i \$PEM_FILE ${server} 'tail -n 100 ${deployPath}/auth_app.log'
        """
    }
}
